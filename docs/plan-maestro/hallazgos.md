...
- **Objetivo de ráfaga autorizado:** el lead acepta como objetivo tres piezas en máximo `3500 ms`. El candidato exige mantener `gamepad1 A + RIGHT_BUMPER`, target `2000 RPM`, cap `0.90`, un burst por INIT y readiness continuo `2000±90 RPM` durante `>=250 ms` antes de cada pieza. Cada pulso conserva `632 ms`, corte duro `700 ms` y cooldown `300 ms`; el intake permanece encendido automáticamente durante PLAY y el servo kicker sigue deshabilitado. Aborta ante release, fault, Stop, más de `3500 ms` desde el primer pulso o batería `<10.50 V`, umbral fail-closed colocado por debajo del mínimo ya validado `10.60 V`. Publica tiempo de inicio y pre/min RPM por pieza.
- **Intake continuo solicitado para la ráfaga:** a petición del lead, `SystemCheck` asigna al intake un comando predeterminado de avance durante todo PLAY, sin botón dedicado. El constructor lo mantiene apagado durante INIT y su shutdown registrado en `RobotSafety` ordena cero ante Stop/E-stop. No se autoriza kicker continuo: mantiene avance por pulsos acotados `632/700 ms` y cooldown, ya que quitar ese límite debilitaría FND-018 y permitiría atasco o alimentación durante la caída de RPM.
- **Candidato de ráfaga pendiente de instalación:** APK `TeamCode-debug.apk`, `81,339,497` bytes, SHA-256 `8C79BBB9DC5801CC9BE381481E1F68DEEE67DB14E980FE7C8EA72EC8CC71387C`; `76/76` tests JVM, cero failures/errors/skips, `assembleDebug` y `git diff --check` PASS. Tres piezas sólo pasan si termina `COMPLETED_3_OF_3` dentro de `3500 ms`, sin anomalía ni salida residual.
...
- **Gate/cierre:** `T8.0 RAW PASS`, T8.1 `1000 RPM PASS`, `1500 RPM PASS`, `2000 RPM PASS`, FND-029 `CLOSED`. El open-loop `0.90` sólo alcanzó `2314.3 RPM`; no avanzar a `2450/2900 RPM`, feeder, piezas ni autos hasta una decisión explícita sobre ampliar nuevamente el envelope o certificar `2000 RPM` como techo actual.
